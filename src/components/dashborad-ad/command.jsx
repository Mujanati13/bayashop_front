      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        <Timeline
          items={events.map(event => ({
            dot: <div className="bg-blue-500 p-2 rounded-full">{event.icon}</div>,
            children: (
              <div className="ml-2">
      ...
      <Modal
        title={
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-t-lg mt-5">
            <div>
              <h3 className="text-xl font-bold">Commande #{selectedOrder.ID_CMD}</h3>
              <p className="text-sm opacity-80">
                <CalendarOutlined className="mr-2" />
                {format(new Date(selectedOrder.Date_cmd), 'dd MMMM yyyy à HH:mm')}
              </p>
            </div>
            <Tag 
                ...
                <div className="space-y-2">
                  {deliveryDetails && Object.keys(deliveryDetails).length > 0 ? (
                    <>
                      ...
                      <p className="text-sm">{JSON.stringify(deliveryDetails)}</p>
                    </>
                  ) : (
                    <div className="flex items-center gap-2 text-amber-600">
                      <ExclamationCircleOutlined />
                      <span className="text-sm">Aucune adresse de livraison</span>
                    </div>
                  )}
                </div>
              </Card>
