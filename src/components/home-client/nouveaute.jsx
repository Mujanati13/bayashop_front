...
const Nouveaute = React.memo(({
  ...
  const endpointUrl = useMemo(() => Endpoint(), []);
 
  const checkQuantity = useCallback(async (requestedQty = 1) => {
    setIsLoading(true);
    try {
      const response = await fetch(`${endpointUrl}/articles/check-quantity/${id}/${requestedQty}`);
  ...
  }, [checkQuantity, quantity, addToCart, id, name, newPrice, image, endpointUrl]);
  ...
  const stockStatusIndicator = useMemo(() => (
    <div className={`absolute top-2 sm:top-3 right-2 sm:right-3 ${getStockStatusClass()} text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium shadow-md`}>
      {stockStatus.available === 0 ? FRENCH_MESSAGES.outOfStock :
       !stockStatus.isAvailable ? `Stock: ${stockStatus.available}` :
       FRENCH_MESSAGES.inStock}
    </div>
  ), [getStockStatusClass, stockStatus.available, stockStatus.isAvailable]);
